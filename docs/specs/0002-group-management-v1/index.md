...
| Preview and join rate limit | Limit, window, account key, IP key, retry delay | `HTTP_INVITE_ATTEMPTS_PER_MINUTE`, one shared UTC epoch minute bucket across both endpoints, authenticated user ID, the direct TCP peer IP supplied by Chi `ClientIPFromRemoteAddr`, and seconds until the next epoch minute with a minimum of one; `X-Forwarded-For`, `Forwarded`, and similar headers are ignored |
...
| `HTTP_RATE_LIMIT_REQUESTS_PER_MINUTE` | Positive request count for the global per IP limiter; default 300. Sized for the busiest legitimate client behind one address (an office NAT, or one device signed into several accounts), since a single app cold start already costs a dozen requests. Health and metrics probes are exempt |
| `HTTP_INVITE_ATTEMPTS_PER_MINUTE` | Positive request count for the invite attempt account plus IP limiter; default 30. Kept separate from the global limiter so loosening ordinary read traffic never widens the invite code guessing window |
