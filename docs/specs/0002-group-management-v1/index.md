...
1. [x] Define group archive or deletion behavior before allowing the sole Captain to leave. Resolved 2026-08-21: **AC-9**, soft archive (`groups.status = 'archived'`), never delete history. This was prompted by `docs/api-change-request.md`'s Disband Group proposal, which had the same `debts.status <> 'settled'` bug as item below and left the delete-vs-archive choice unresolved; both are settled here.
